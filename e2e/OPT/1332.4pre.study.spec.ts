...
const preVisitedPages = [ 
  {name: 'Homepage', url: 'https://www.samsung.com/uk/'},
  {name: 'Smartphones PLP', url: 'https://www.samsung.com/uk/smartphones/'},
  {name: 'Offers page', url: 'https://www.samsung.com/uk/offers/'},
  {name: 'Support page', url: 'https://www.samsung.com/uk/support/'},
];
async function assertCopy(page, copy: string, subcopy: string) {
...
async function testItself(page, url: string, copy: string, subcopy: string, altText: string, disclaimerCopy: string) {
  const card = await assertCopy(page, copy, subcopy);
  await assertAltText(card, altText);
  await assertDisclaimer(page, disclaimerCopy);
  await assertLinkedURL(page, url);//exclude for mobile
}
